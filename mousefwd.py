...
parser.add_argument('-s', '--sensitivity', help='mouse sensitivity (default 1.0)', default=1, type=float)
...
if subprocess.run(['scp', 'receiver.py', 'cursor.png', f'{args.host}:{args.receiver_dir}/']).returncode == 0:
    ...
    sender = Sender(connection.stdin, args.cursor, *parseHotkey(args.hotkey), args.sensitivity)
    sender.run()
else:
